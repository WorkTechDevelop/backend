...
public interface SprintsRepository extends JpaRepository<Sprint, String> {
    ...
    @Query("SELECT NEW ru.worktechlab.work_task.model.response_dto.SprintInfoDTO(s.name, s.startDate, s.endDate) FROM Sprints s WHERE s.projectId = :projectId AND s.active = true")
